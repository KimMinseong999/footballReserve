...
public class KakaoLogin {
	public static JsonNode getAccessToken(String autorize_code) {
		...
		postParams.add(new BasicNameValuePair("redirect_uri", "http://localhost:8000/controller/sign/kakaologin")); // 리다이렉트 URI
...
/*			System.out.println("\nSending 'POST' request to URL : " + RequestUrl);
	...
	public static UserDTO changeData(JsonNode userInfo) {
		...
		vo.setUserId(userInfo.path("id").asText()); // id -> vo 넣기

		if (userInfo.path("kaccount_email_verified").asText().equals("true")) { // 이메일 받기 허용 한 경우
			vo.setUserEmail(userInfo.path("kaccount_email").asText()); // email -> vo 넣기
